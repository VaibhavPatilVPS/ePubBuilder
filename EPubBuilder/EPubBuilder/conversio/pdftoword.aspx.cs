...
namespace iCE_Webapp_Bootstrap.conversio
{
    ...
    {
        string _product = "VPS-PDF2WORD";
        bool _chkstatus = false;
        ...
        public void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dropeq.SelectedValue.ToString() == "AutoZoning")
                {
                    root.Add(new XElement("product", "AutoZoning"));
                    _type = "AutoZoning";
                }
                else
                    root.Add(new XElement("product", "CONVERSIO_PDF2Word"));
                ...
                Debug.WriteLine(dropeq.SelectedValue.ToString());


                if (_type == null)
                {
                    root.Add(new XElement("EquationAsImage", dropeq.SelectedValue.ToString()));
                    
                    if (dropeq.SelectedValue.ToString() == "True")
                        _type = "P2W_EquationImage";
                    else
                        _type = "P2W_EquationMath";
                }
                else
                    root.Add(new XElement("EquationAsImage", "True"));
            ...
            if (inputuploadsingle.HasFile)
            {
                ...
                for (int i = 0; i < hfc.Count; i++)
                {
                    ...
                    if (hpf.ContentLength > 0)
                    {
                        ...
                        {
                            string fileName = server + "\\" + hpf.FileName.Replace(" ", "_");
                            //string fileName = server + "\\" + _timestamp +
                            //    Path.GetExtension(hpf.FileName);
                            string ZipName = server + "\\" + _timestamp + "_" + Path.GetFileNameWithoutExtension(hpf.FileName).Replace(" ", "_") +
                                    "_Conversio_PDF2Word.zip";

                            hpf.SaveAs(fileName);

                            AddExcelToZip(newXMLFile, fileName, ZipName);

                            try{File.Delete(newXMLFile);}catch { }
                            ...
                            sql = @"INSERT INTO job_pdf2word(id, job_id, product_code, user_id, file_name, input_file_name, vm_id, product_id, input_time, status, type) VALUES(" +
                                "nextval('job_pdf2word_Inc'),'" + _timestamp + "','" + _product + "','" +
                                Session["userid"].ToString() + "', '" + Path.GetFileName(ZipName) + "', '" + Path.GetFileName(hpf.FileName) + "', '1','" + _product_ID
                        ...
                        else
                        {
                            ...
                            sql = @"INSERT INTO job_conversio(id, job_id, product_code, user_id, file_name, vm_id, product_id, input_time, status) VALUES(" +
                                "nextval('job_conversio_Inc'),'" + _timestamp + "','" + _product + "','" +
                                Session["userid"].ToString() + "', '" + Path.GetFileName(hpf.FileName) + "', '1','" + _product_ID
                                + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")
                                + "','Extn-Error');";
            ...
            File.Delete(newXMLFile);
            Response.Redirect("pdftoword.aspx");
            ////////////////////////////////////////
        }

        #region Add Excel to zip
        private void AddExcelToZip(string xml, string _pdf, string zipPath)
        {
        ...
        private void BindListView()
        {
            try
            {
                ...
                string sql = @"Select input_file_name,status,isbn,page_count,input_time, output_time, output_file_name, type from job_pdf2word WHERE (user_id=" + Session["userid"].ToString() + ") ORDER BY id DESC;";
                ...
                newdt.Columns.Add(new DataColumn("type", typeof(string)));
                int _sl = 1;
                if (dt.Rows.Count > 0)
                {
        ...
        //public void EqnCheked_Changed(object sender, EventArgs e)
        //{
        //    if (eqnyes.Checked == true)
        //        _chkstatus = true;
        //    else if (eqnyes.Checked == false)
        //        _chkstatus = false;
        //}
    }
}